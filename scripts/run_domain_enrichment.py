#!/usr/bin/env python3
"""Convenience wrapper for invoking the corporate domain enrichment lambda locally."""
...
def select_run_id(client: firestore.Client, explicit: Optional[str]) -> Optional[str]:
    ...
    query = client.collection("enrichment_runs").where("stage", "==", "domain")
    runs = list(query.stream())
    if not runs:
        logging.warning("No enrichment run awaiting domain stage")
        return None
    runs.sort(key=lambda doc: doc.get("completed_at") or doc.get("created_at"), reverse=True)
    run = runs[0]
    logging.info("Selected run %s for domain enrichment", run.id)
    return run.id
...
def main() -> None:
    ...
    parser.add_argument(
        "--run-id",
        default=None,
        help="Specific enrichment_runs document to process (default: newest stage=domain).",
    )
    parser.add_argument(
        "--env-file",
        default=None,
        help="Optional .env file to load before running (default: services/enrichment-domain-lambda/.env).",
    )
    parser.add_argument(
        "--credentials",
        default=None,
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Enable ENRICHMENT_DRY_RUN=1 so Anthropic is skipped.",
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
    ...
    service_dir = services_root / "enrichment-domain-lambda"
    ...
    package_name = "enrichment_domain_lambda"
    ...
    if package_spec is None or package_spec.loader is None:
        raise ImportError("Unable to create module spec for domain package")
    ...
    try:
        ...
        logging.error("Failed to import domain enrichment handler from %s: %s", service_dir, exc, exc_info=True)
        raise
    ...
    logging.info("Invoking domain enrichment with payload: %s", payload)
    handler(payload, None)
    logging.info("Domain enrichment invocation complete.")
